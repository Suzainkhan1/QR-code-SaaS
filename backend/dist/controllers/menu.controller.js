...
exports.deleteItem = exports.updateItem = exports.addItem = exports.deleteCategory = exports.updateCategory = exports.addCategory = exports.getCategories = exports.getMenuBySlug = void 0;
...
const addCategory = async (req, res) => {
    ...
    try {
        const existing = await db_1.prisma.menuCategory.findFirst({
            where: { restaurantId, name: { equals: name } },
        });
        if (existing) {
            return res.status(400).json({ error: 'Category name already exists' });
        }
        const count = await db_1.prisma.menuCategory.count({ where: { restaurantId } });
        const newCategory = await db_1.prisma.menuCategory.create({
            data: {
                name,
                description,
                sortOrder: count,
...
const updateCategory = async (req, res) => {
    ...
    if (!restaurantId)
        return res.status(401).json({ error: 'Unauthorized' });
    try {
        const category = await db_1.prisma.menuCategory.findFirst({
            where: { id, restaurantId },
        });
        if (!category) {
            return res.status(404).json({ error: 'Category not found' });
        }
        const updated = await db_1.prisma.menuCategory.update({
            where: { id },
...
const addItem = async (req, res) => {
    ...
    try {
        ...
        const newItem = await db_1.prisma.menuItem.create({
            data: {
                name,
                description,
...
const updateItem = async (req, res) => {
    ...
    try {
        ...
        const updated = await db_1.prisma.menuItem.update({
            where: { id },
            data: {
                name,
                description,
